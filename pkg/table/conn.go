...
func ChugakuReceive(client *conn.ChugakuClient, rows []string, cols []string) (*Table, error) {
	data, err := client.ReceiveTable()
	if err != nil {
		return nil, err
	}
	return New(data, rows, cols), nil
}
...
func YobikouReceive(server *conn.YobikouServer, rows []string, cols []string) (*Table, error) {
